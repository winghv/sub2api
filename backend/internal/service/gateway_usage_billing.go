...
type RecordUsageInput struct {
	...
	// ParsedRequest 可选：Claude Max 计费策略辅助判定用（fork 定制）。
	// upstream 异步计费 worker 刻意不持有 ParsedRequest；handler 后扣链路保持 nil，
	// 实际 usage 投影在同步响应/流式路径完成。此字段仅供 claude max 策略 helper/测试复用。
...
type recordUsageCoreInput struct {
	...
	ParsedRequest      *ParsedRequest // 可选：Claude Max 计费投影用（fork 定制），异步/Gemini 路径为 nil
	ChannelUsageFields
}
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	//
	// Claude Max 计费（fork 定制）：claude-max 分组将 usage 投影为 1h 缓存写入，并绕过
	// cache-TTL override（否则会把已投影的 1h token 错误重映射回 5m）。投影需要 ParsedRequest
	// 的缓存信号：同步响应/流式路径传入 input.ParsedRequest；异步后扣快照为 nil 时，若 usage
	// 已在同步路径投影则命中 hasCacheCreationTokens 提前返回，不会重复投影。
	cacheTTLOverridden := false
	claudeMaxBillingRulesApplied := shouldApplyClaudeMaxBillingRulesForUsage(apiKey.Group, result.Model, input.ParsedRequest)
	if claudeMaxBillingRulesApplied {
		applyClaudeMaxCacheBillingPolicyToUsage(&result.Usage, input.ParsedRequest, apiKey.Group, result.Model, account.ID)
	}
	if !claudeMaxBillingRulesApplied {
