...
// 用于处理客户端非流式请求但上游只支持流式的情况
// accountID 为 fork 定制：Claude Max 模拟计费需要账号维度
func (s *AntigravityGatewayService) handleClaudeStreamToNonStreaming(c *gin.Context, resp *http.Response, startTime time.Time, originalModel string, accountID int64) (*antigravityStreamResult, error) {
	...
	// Claude Max cache billing simulation (non-streaming, fork 定制)
	if streamRes != nil && streamRes.usage != nil {
		agUsage := &antigravity.ClaudeUsage{
			...
			ImageOutputTokens:        streamRes.usage.ImageOutputTokens,
		}
