...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if isCodexCLI && codexImageGenerationExplicitToolPolicy == codexImageGenerationExplicitToolPolicyStrip {
	...
	} else if isCodexCLI && !imageGenerationAllowed {
		// 分组不允许生图（fork 定制）：Codex CLI 默认给每个请求附带 image_generation 工具，
		// 若原样转发，上游会对普通请求也返回 403（"Image generation is not enabled for this
		// group"）。这里剥离客户端自带的工具，让普通 Codex 请求正常转发。注意仅在生图未开启
		// 时剥离——生图已开启的分组即便桥接关闭，显式 image_generation 工具仍应放行（见
		// TestOpenAIGatewayServiceForward_ExplicitImageToolWorksWithBridgeDisabled）。账号
		// 策略 strip 的场景由上面的分支处理。
		decoded, decodeErr := ensureReqBody()
		if decodeErr != nil {
			return nil, decodeErr
		}
		if stripOpenAIImageGenerationTools(decoded) {
			markDecodedModified()
			logger.LegacyPrintf("service.openai_gateway", "[OpenAI] Stripped client image_generation tool (image generation disabled for group) for Codex client")
		}
		imageIntent = IsImageGenerationIntentMap(openAIResponsesEndpoint, reqModel, decoded)
	} else {
